export function FestivalHeader({ teacher, setTeacher }: { teacher: boolean; setTeacher: (v: boolean) => void }) {
  return (
    <header className="hero">
      ...
      <label className="switch"><input aria-label="Turn Teacher Mode on or off" type="checkbox" checked={teacher} onChange={(e: any) => setTeacher(e.target.checked)} /> Teacher Mode</label>
    </header>
  );
}
